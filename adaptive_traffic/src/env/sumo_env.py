...
class SumoEnv:
    ...
    Uses TraCI to interface with SUMO simulation.
    Observation: queue lengths for each incoming direction (4,)
    Action: discrete index selecting green duration for current phase
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.observation_space = Box(low=0, high=self.queue_capacity * 4, shape=(self.num_directions,), dtype=np.int32)  # Assuming 4 lanes per edge
    ...
    def reset(self, *, seed: int | None = None, options: Dict[str, Any] | None = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        return queues.astype(np.int32), info
